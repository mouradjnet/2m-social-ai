...
3. O Render vai pedir os dois segredos marcados como `sync: false`:
...
- **`config:cache` no build não funciona** — em build time não existe `DB_HOST` nem `APP_KEY`. Por isso os caches são gerados no `entrypoint.sh`, com o ambiente já injetado.
